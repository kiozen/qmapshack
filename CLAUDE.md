...
- **Record the intent, replay the input, check the outcome.** Checkables record `checked`; a combo
  ...
  a tab is `currentChanged` (a close retracts the switch it caused); a moved dock is `arrange` with
...
- **A menu entry is addressed by its action's `objectName`, never its text.** Name actions after the
  ...
  `actionColor_<GPX name>`, `actionWptIcon_<sym>`). POI entries: `IPoiItem::file`/`key`, a map POI by
  name and position. Every new menu entry needs one; nothing fails when it is missing — the recorder
  warns and drops the step.
...
- **An empty screen name is not passed as `--doc-screen`** (offscreen screens have none).
...
- **The state re-applies `MainWindow/geometry` after the fixture is up**, and
  ...
  not drop it on another screen size (>25 % difference). `moveToWritersScreen()` centres the window
  on the panel's screen (`--doc-screen`). Measured: a base stored at 1200x876 on a 1700 px screen came
...
- **Render before asking anything.** Focus cannot be restored across another window, and a project
  ...
  (`livePartsAt()`) at the key press. `CShotRegionPicker` takes `Qt::NoFocus`.
...
- **Take again is headless**: the launcher runs `shots.py -o doc/images/_work replay --only <id>`,
  what a build does. `replay` deletes `<out>/<id>.png` first, so a failure loses the work picture.
