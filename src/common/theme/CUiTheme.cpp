...
void CUiTheme::pinColorScheme(bool dark) {
  ...
  // The seven-colour ctor takes BrightText from `light`, which is #4b4b4b on a dark window.
  ...
  // Fusion's light value is a literal and stays one, so the light arm matches standardPalette()
  // exactly; on a dark window #bebebe would be a near-white edge, so that arm derives.
