  ...
  void watch(QObject* object, CShotRecorder& recorder) const override {
    ...
    QObject::connect(bar, &QTabBar::currentChanged, &recorder, [bar, scope, &recorder](int index) {
      ...
      if (index >= 0 && address.has_value()) {
        recorder.record(scope, CShotStep::set(*address, "currentIndex", index));
      }
    });
    QObject::connect(bar, &QTabBar::tabCloseRequested, &recorder, [bar, scope, &recorder](int index) {
      const std::optional<QString>& address = recorder.address(bar);
      if (!address.has_value()) {
        return;
      }
      ...
      recorder.retractLast(
          scope, [](const QJsonObject& step, bool sameFrame) { return sameFrame && "set" == step["do"].toString(); });
      recorder.record(scope, CShotStep::closeTab(*address, index));
    });
  }
 ...
 public:
  ...
  void watch(QObject* object, CShotRecorder& recorder) const override {
    ...
    view->viewport()->installEventFilter(
        new CPressWatch(&recorder, [doubled](const QMouseEvent*) { *doubled = QPersistentModelIndex(); }));
...
class CRowButtonHandler : public CItemViewHandler {
 public:
  void watch(QObject* object, CShotRecorder& recorder) const override {
    ...
    tree->viewport()->installEventFilter(new CPressWatch(&recorder, [this, tree, press](const QMouseEvent* event) {
