...
void CShotDocMode::moveToWritersScreen() {
  // A stored geometry carries a position on the whole desktop; the window belongs on the panel's screen.
  if (qlOpts->doc.docScreen.isEmpty() || CMainWindow::isNull()) {
    return;
  }
  const QList<QScreen*>& screens = QGuiApplication::screens();
  for (const QScreen* screen : screens) {
    if (screen->name() == qlOpts->doc.docScreen) {
      CMainWindow& window = CMainWindow::self();
      // move() places the frame.
      QRect frame = window.frameGeometry();
      frame.moveCenter(screen->availableGeometry().center());
      window.move(frame.topLeft());
      qDebug() << "doc: window on" << screen->name() << window.frameGeometry();
      return;
    }
  }
}
...
void CShotDocMode::tag() {
  ...
  if (isPartOf(main, target)) {
    ...
    for (QWidget* part : std::as_const(parts)) {
      // A window is rendered at its own size, never at its size hint.
      pictures.insert(part, CShotWriter::render(part, part->isWindow() ? part->size() : QSize()));
    }
  }
