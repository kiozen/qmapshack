...
- **The start state comes first**: the leading `layout` (`restoreState()`) and `view`, then
  ...
  the draw thread.
- **The `tab` index and splitter states are applied after the last step**: a page a step adds does
  not exist while `restoreState()` runs, and `QTabWidget` drops an index past its last page.
